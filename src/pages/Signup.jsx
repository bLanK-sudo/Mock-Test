                    ...
                    <div class="card bg-base-300 m-4">
                        ...
                        <div class="card-body flex flex-col gap-4">
                            <div class="card-title flex-col items-start">
                                ...
                                <input class="input input-bordered input-accent w-full" type="text" name="name" id="name" required/>
                                <label for="name">Email</label>
                                <input class="input input-bordered input-accent w-full" type="email" name="email" id="email" required/>           
                                <label htmlFor="pwd">Password</label>
                                <input class="input input-bordered input-accent w-full" type="password" name="pwd" id="pwd" required/>
                                <label htmlFor="bio">Bio</label>
                                <textarea class="textarea textarea-bordered textarea-accent w-full" name="bio" id="bio" cols="30" rows="4"></textarea>
                                <label htmlFor="level">Level</label>
                                <select class="select select-accent w-full" name="level" id="level">
                                ...
                                <select onChange={(el) => {setonclick(el)} } class="select select-accent w-full" id="courses" name="test">
                                    <option disabled selected hidden >Choose a subject</option>
                                    <For each={courses()}>{
                                        (course) => {
                                            return <option  name={course.name} value={course.id}>{course.name}</option>
                                        }
                                    }</For>
                                </select>
                                </Show>
                                <div class="flex gap-2 flex-wrap w-full">
                                    <For each={selectedCourses}>{
                                            (course, i) => {
                                                return <div class="flex justify-between gap-2 border border-accent">
                                                    ...
                                                    } class="text-lg w-8 border-l-2 border-accent h-full flex justify-center items-center">X</button>
                                                </div>
                                            }
                                    }</For>
                                </div>
                            ...
                            <button type="submit" ref={signUpBtn} class="btn btn-accent w-full"  onClick={async() => {
                                if(document.getElementById("name").value == "" || document.getElementById("pwd").value == "" || document.getElementById("email").value == "" || document.getElementById("bio").value == "" ) {
                                ...
                                signUpBtn.innerHTML = "<div class='loading loading-spinner loading-sm'></div> Signing Up...";
                                let courseArr = [];
                                for (let i = 0; i < selectedCourses.length; i++) {
                            ...
                            <div class="p-4">Don't have an account? <A href="/login" class="link">Login</A></div>
                            </div>
                        </div>
                    </div>
